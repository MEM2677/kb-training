...
public class TableauTag extends TagSupport {

	private static final Logger _logger =  LoggerFactory.getLogger(TableauTag.class);
	...
	public int doStartTag() throws JspException {
		...
		Tableau tableau = null;
			if (null != this.getKey()) {
				tableau = tableauManager.getTableau(this.getKey());
			} else {
				Widget widget = this.extractWidget(reqCtx);
				ApsProperties widgetConfig = widget.getConfig();
				String varid = widgetConfig.getProperty("id");
				if (StringUtils.isNotBlank(varid)) {
					tableau = tableauManager.getTableau(new Integer(varid));
				}
			}
			this.pageContext.setAttribute(this.getVar(), tableau);
		} catch (Throwable t) {
			_logger.error("Error in doStartTag", t);
			throw new JspException("Error in TableauTag doStartTag", t);
		}
